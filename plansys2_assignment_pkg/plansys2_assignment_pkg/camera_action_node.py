...
class CameraAction(ActionExecutorClient):

    def __init__(self):
        super().__init__('camera_action', 0.5)
...
##############################################################################
    ...
    def timer_callback(self):            
        ...
        if self.flag == 0:
            ...
            self.send_feedback("Searching marker {0}".format(self.id_marker))
            self.aruco_controller_area()
        elif self.flag == 1:
            self.iteration = 0
            self.send_feedback("I'm going to marker {0}".format(self.id_marker))
        ...
        if self.flag_marker == 1 and len(self.corners_marker) != 0:                    
            ...
            if biggest_side > distance_check:
                ...
                if self.reached_marker == 4:
                    self.finish(True, 1.0, "All markers reached")
                    self.destroy_node()
                    rclpy.shutdown() 
                self.finish(True, 1.0, "N° markers reached:{0} \n Marker reached:{1}".format(self.reached_marker, self.id_marker))                       
        else:
...
def main(args=None):
    ...
    camera_action.set_parameters([Parameter(name='action_name', value='camera_action')])
    
    camera_action.trigger_configure()
    
    rclpy.spin(camera_action)
